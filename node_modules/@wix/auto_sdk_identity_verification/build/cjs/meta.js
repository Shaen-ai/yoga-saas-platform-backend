...
// src/iam-verification-v1-start-response-verification.types.ts
...
// src/iam-verification-v1-start-response-verification.meta.ts
function start2() {
  const payload = {};
  const getRequestOptions = start(payload);
...
function verifyDuringAuthentication2() {
  const payload = {};
  const getRequestOptions = verifyDuringAuthentication(
    payload
  );
  const getUrl = (context) => {
    const { url } = getRequestOptions(context);
